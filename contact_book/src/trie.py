...
class Trie:
    ...
    def insert(self, root, key):
        ...
        node = TrieNode(key[0])
        if not root.children[indexToInsert]:
            root.children[indexToInsert] = node
        rem = key[1:]
        self.insert(root.children[indexToInsert], rem)
        return root

    def search(self, root, key):
        index = self._charToIndex(key[0])
        node = root.children[index]
        return self.someS(0, key, node)

    def someS(self, index, key, root):
        if index == len(key) - 1 and key[index] == root.char:
            return root
        if index > len(key) or key[index] != root.char:
            return None
        nodeIndex = self._charToIndex(key[index+1])
        node = root.children[nodeIndex]
        return self.someS(index + 1, key, node)
